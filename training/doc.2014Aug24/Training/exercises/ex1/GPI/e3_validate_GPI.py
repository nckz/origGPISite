...
class ExternalNode(gpi.NodeAPI):
    ...
    def validate(self):
        ...
        # check some requirement of the data that is not
        # enforcible by the ports
        data = self.getData('in-data')
        if data.min() <= -0.1:
            # E3: Make the warning reflect the 'if' condition.
            self.log.warn(
                'The input data is not suited for this algorithm!  Aborting.')
            return 1  # warning
    ...
    def compute(self):
        ...
        if out.max() >= 2.1:
            # E3: Make the warning reflect the 'if' condition.
            self.log.warn('The results exceed the threshold.')
            return 1  # warning
